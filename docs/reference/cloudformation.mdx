...
|---|---|---|
| empty (new VPC) | `true` or `false` | Always creates a stack-owned exclusion; the parameter is ignored because a new VPC always needs one |
| set | `true` | Creates an exclusion with `DeletionPolicy: Retain` and `UpdateReplacePolicy: Retain`, so the shared VPC-wide exclusion outlives this stack |
| set | `false` | Creates nothing; the VPC must already have a complete `allow-bidirectional` exclusion |
